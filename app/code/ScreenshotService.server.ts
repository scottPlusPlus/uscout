...
export default async function getScreenshot(url: string): Promise<string> {
  ...
  const endpoint = `https://api.screenshotone.com/take?access_key=${API_KEY}&url=${encodedUrl}&device_scale_factor=1&format=jpg&block_ads=true&block_cookie_banners=true&block_trackers=true&cache=true&cache_ttl=604800`;
